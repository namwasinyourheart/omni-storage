...
If `MODAL_CREDENTIALS` is not provided, it uses the active Modal profile on your system.

**Example Setup:**
```bash
# Required: Modal Volume name
export MODAL_VOLUME_NAME="my-volume"

# Optional: Path to credentials for auto-login
export MODAL_CREDENTIALS="/path/to/credentials.json"
```
...
# Automatic detection (when MODAL_VOLUME_NAME is set)
storage = get_storage()
...
1. **Explicitly via `storage_type` parameter**: Pass `storage_type="s3"`, `storage_type="gcs"`, `storage_type="modal"`, or `storage_type="local"` to `get_storage()`
