...
def get_storage(
    storage_type: Optional[Literal["s3", "gcs", "local", "modal"]] = None,
) -> Storage:
    ...
    if storage_type:
        if storage_type not in ["s3", "gcs", "local", "modal"]:
            raise ValueError(
                f"Invalid storage_type: {storage_type}. Must be 's3', 'gcs', 'local', or 'modal'."
            )
        ...
        if os.getenv("AWS_S3_BUCKET"):
            ...
            chosen_type = "gcs"
        elif os.getenv("MODAL_VOLUME_NAME"):
            chosen_type = "modal"
        else:
            chosen_type = (
    ...
    elif chosen_type == "modal":
        volume_name = os.getenv("MODAL_VOLUME_NAME")
        if not volume_name:
            raise ValueError(
                "MODAL_VOLUME_NAME environment variable is required for Modal storage."
            )
        from .modal import ModalStorage  # Lazy import ModalStorage

        return ModalStorage(volume_name)
